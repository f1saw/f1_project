...
if (!check_admin_auth($user)) {
    error("401", "not_authorized", "\\views\private\user_delete.php", "/f1_project/views/public/auth/login.php", "Unauthorized access.");
    exit;
}
set_session($user);
...
$conn = DB::connect("\\views\private\user_delete.php", "/f1_project/views/private/user_delete.php");

// TODO: check_user_role() equivale a check_admin_auth() ?
$check_role = check_user_role($conn,
    [$_GET["id"]],
    "\\views\private\user_delete.php",
    "/f1_project/views/private/user_delete.php");

if (!$conn->close()) {
    error("500", "conn_close()", "views\private\user_delete.php", "/f1_project/views/private/dashboard.php");
    exit;
}
...
if (!$conn->close()) {
    error("500", "conn_close()", "\\views\private\user_delete.php", "/f1_project/views/private/user_delete.php");
    exit;
}
